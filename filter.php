...
if(isset($_REQUEST["search_by"]) && isset($_REQUEST["search"]) ){
...
//  die($search_by === 'all');
  ...
  }elseif ($search_by == 'event_date'){
    $sql = "SELECT * FROM events where {$search_by} like '%{$search}%' LIMIT {$offset},{$limit_per_page}";
  } else{
    $sql = "SELECT * FROM events where {$search_by} like '%{$search}%' LIMIT {$offset},{$limit_per_page}";
